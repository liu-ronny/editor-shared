...
export class DiffRange {
    ...
    constructor(diffRangeType: DiffRangeType, diffHighlightType: DiffHighlightType, start: DiffPoint, stop: DiffPoint) {
    ...
    static fromDiff(diffRangeType: DiffRangeType, source: string, index: number, length: number): DiffRange {
        ...
        if (index + length == source.length - 1 || source[index] == '\n') {
            ...
            // consume the entire line if we're deleting an entire line
            if (start[0] == stop[0]) {
                stop[0]++;
                stop[1] = 0;
            }
